...
# 必要な関数のインポート
import numpy as np
from sklearn.metrics import roc_curve, roc_auc_score  # ROC曲線を作成するための関数
...
# memo-------------------------------------------------------------------------
# とりあえず不要な関数（バージョンアップしていく中で使うかもしれないのでコメントアウト）
# import time
# import re
...
# import re
# from scipy.special import comb, perm  # 組み合わせ馬券の計算に使用(nCr nPr)
# # pycaretライブラリのインポート
...
GET_DATA_YEAR = "2017-2022"  # ここで指定した文字列のディレクトリをresult_dataディレクトリ直下に作成してそこに学習結果を保存する
...
class Preprocessing_Horse_Result:
    ...
    def preprocessing_horse_result(self):
        ...
        """
        # memo------------------------------------------------------------------
        # pd_horse_results = df[["着順", "賞金", "日付", "着差", "通過", "開催", "距離"]].copy()
        # df = pd_horse_results.copy()
        # df["horse_id"] = df.index  # columnにhorse_idを一時的に追加(次の処理で重複行を削除するときに消したらいけない行を消さないため)
        # df = df.drop_duplicates().drop("horse_id", axis=1).copy()  # 重複行の削除とhorse_idの列を削除
        # memo------------------------------------------------------------------
...
class Return:
    ...
    @classmethod
    def load_pickle(cls, GET_DATA_YEAR_LIST):
        # あらかじめスクレイピングしてある馬券ごとの払い戻しデータを読み込む(pd_return_tables_〇〇〇〇: DataFrame型)
        df = pd.concat([load_pickle(FILE_PATH_BASE_DATA, f"pd_return_tables_{GET_DATA_YEAR}") for GET_DATA_YEAR in GET_DATA_YEAR_LIST])
...
class Calucurate_Return:
    ...
    def __init__(self, model, GET_DATA_YEAR_LIST, X_test_add_tansho_ninnki, X_test, baken_type, is_standard_scarer=True, is_use_pycaret=True):
        ...
        # rt = Return.load_pickle(GET_DATA_YEAR_LIST)
        # X_test_add_tansho_ninnki.index.unique()
        # return_tables = return_tables
        # return_tables.columns = ["type", "win", "return", "人気"]
        # memo------------------------------------------------------------------
        self.model = model
...
class Result_Table(Data_Processer):
    ...
    def load_pickle(cls, GET_DATA_YEAR_LIST):
        ...
        # (注意)pd_race_resultsはdrop_duplicatesをしてはいけない(race_idの被りはなくてもシンプルにinfoが同一の場合があるため)
    ...
    def preprocessing(self):
        ...
        df["体重変化"] = pd.to_numeric(df["馬体重"].str.split("(", expand= True)[1].str[:-1], errors="coerce").fillna(0).astype(int) # 計測していない馬は"計不"となるためそれを検知して0に変換している
...
def main():
    ...
    # accuracy(的中率) のグラフ描写(各馬券ごとにまとめている)
    multi_graph_plot(accuracy_diff_single_list, accuracy_model_single_list, graph_title_single_list, graph_main_title=f"accuracy_single_{GET_DATA_YEAR}", x_label="trade_count", y_label="accuracy(%)", rows=1, columns=2, figsize_width=20, figsize_height=5)
    ...
    # return(回収率) のグラフ描写(各馬券ごとにまとめている)
