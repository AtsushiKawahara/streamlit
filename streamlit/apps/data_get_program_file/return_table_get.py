...
def main():
    ...
    for GET_DATA_YEAR in GET_DATA_YEAR_LIST:
        ...
        if os.path.isfile(f"{FILE_PATH_BASE_DATA}/pd_return_tables_test_{GET_DATA_YEAR}"):
            print(f"pd_return_tables_test_{GET_DATA_YEAR}　is exist")

            pd_return_tables_test_load = load_pickle(FILE_PATH_BASE_DATA, f"pd_return_tables_test_{GET_DATA_YEAR}")  # 途中まで抽出しているファイルを読み込み
        ...
        # 報酬情報の保存と読み込み
        save_pickle(FILE_PATH_BASE_DATA, f"pd_return_tables_test_{GET_DATA_YEAR}", pd_return_tables_test)
        # ---------------------------------------------------------------------------
